...
export default defineEventHandler(async (event) => {
  try {
    const body = await readBody<{ name?: string; description?: string; price?: number; image?: string }>(event)
    if (!body.name || !body.description || typeof body.price !== 'number') {
      throw createError({ statusCode: 400, statusMessage: 'Bad Request: name, description and price are required' })
    }
