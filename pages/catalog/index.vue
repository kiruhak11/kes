...
if (
  ...
  categories.value = (fetchedCategories.value.categories as any[]).map(
    (cat: any) => ({
      ...cat,
      currentImageIndex: 0,
    })
  ...
  categories.value = ((fetchedCategories.value as any).categories as any[]).map(
    (cat: any) => ({
      ...cat,
      currentImageIndex: 0,
    })
  ...
  if (fetchError.value) {
    ...
    console.error(
      "Некорректный ответ от /api/categories:",
      fetchedCategories.value
    );
  }
}
